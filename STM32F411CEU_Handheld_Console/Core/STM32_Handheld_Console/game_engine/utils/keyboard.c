...
const char kb_layout_lowercase[4][10] = {
	{ '1', '2', '3', '4', '5', '6', '7', '8', '9', '0'  },
	{ 'q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p'  },
	{ 'a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', '\'' },
	{ 'z', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', '?'  },
...
const char kb_layout_uppercase[4][10] = {
	{ '!', '@', '#', '$', '%', '^', '&', '*', '(', ')'  },
	{ 'Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P'  },
	{ 'A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L', '"'  },
	{ 'Z', 'X', 'C', 'V', 'B', 'N', 'M', ';', ':', '-'  },
...
const char kb_layout_symbols1[4][10] = {
	{ '1', '2', '3' , '4', '5', '6', '7', '8', '9', '0'  },
	{ '@', '#', '$' , '_', '&', '-', '+', '(', ')', '/'  },
	{ '*', '"', '\'', ':', ';', '!', '?', ',', '.', '\\' },
	{ '~', '|', '<' , '>', '{', '}', '[', ']', '^', '`'  },
...
const char kb_layout_symbols2[4][10] = {
	{ CHR_TR_C    , CHR_TR_O    , CHR_TR_I   , CHR_TR_S   , CHR_TR_U , CHR_TR_G, 127     , 127     , 127        , 127           },
	{ CHR_TR_c    , CHR_TR_o    , CHR_TR_i   , CHR_TR_s   , CHR_TR_u , CHR_TR_g, 127     , 127     , 127        , 127           },
	{ CHR_GP_UP   , CHR_GP_RIGHT, CHR_GP_DOWN, CHR_GP_LEFT, CHR_GP_X , CHR_GP_Y, CHR_GP_A, CHR_GP_B, CHR_GP_HOME, CHR_GP_SELECT },
	{ CHR_GP_START, CHR_GP_L1   , CHR_GP_L2  , CHR_GP_R1  , CHR_GP_R2, 127     , 127     , 127     , 127        , 127           },
};

char text_buffer[MAX_INPUT_LEN + 1];
...
{
	if(selected_col == 0) selected_col = 9;
	else selected_col--;
}

void go_right_on_keyboard()
{
	if(selected_col == 9) selected_col = 0;
	else selected_col++;
}
...
{
	fill_raw_rect(0, KEYBOARD_POS_Y, 240, 70, 0);
...
void render_input_box(char *title, uint8_t cursor_on)
{
	fill_raw_rect(0, 149, 240, 11, system_settings.theme_color_primary);
	set_text_color(system_settings.theme_color_secondary, system_settings.theme_color_secondary);
	draw_formatted_text(1, 155, TEXT_H_ALIGN_LEFT | TEXT_V_ALIGN_CENTER, STR_GP_A"%s "STR_GP_START"%s", get_str(STR_ENTER_LETTER), get_str(STR_CANCEL));
...
void move_cursor_to_left()
{
	if(text_cursor_index > 0)
	{
		...
		if(text_cursor_x < 0)
		{
			if(text_cursor_x + cw > 0) text_offset -= cw - text_cursor_x;
			else text_offset -= cw;
...
void enter_letter(char letter)
{
	if(text_len < max_text_len && letter != 127)
	{
		if(text_cursor_index < text_len)
		{
			memcpy(&text_buffer[text_cursor_index + 1], &text_buffer[text_cursor_index], strlen(&text_buffer[text_cursor_index]));
		}
...
void show_input_screen(RAM_PTR dest, uint8_t max_len, char *title)
{
	...
	for(;;)
	{
		...
		// Uppercase key
		if(get_key_down(GAMEPAD_L2)) kb_flags ^= (kb_flags & KB_SYMBOLS) ? KB_SYMBOLS2 : KB_UPPERCASE;
		...
		{
			memcpy(&ram[dest], text_buffer, text_len + 1);
			return;
		}
