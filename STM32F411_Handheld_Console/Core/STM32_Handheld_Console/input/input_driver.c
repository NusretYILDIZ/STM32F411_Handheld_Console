...
const uint8_t *update_string = "u";
//extern uint8_t input_buffer[6];

void read_usb_vcp(uint8_t *buf)
{
	//while(CDC_Transmit_FS("u", 1) != USBD_OK);
	
	previous_keys = current_keys;

	current_keys = *(uint16_t *)buf;
	
	axises[0] = *(int8_t *)(buf + 2);
	axises[1] = *(int8_t *)(buf + 3);
	axises[2] = *(int8_t *)(buf + 4);
	axises[3] = *(int8_t *)(buf + 5);
}
...
void update_inputs()
{
	...
	while(CDC_Transmit_FS((uint8_t *)update_string, 1) != USBD_OK);
	//CDC_Transmit_FS((uint8_t *)update_string, 1);
	//HAL_Delay(1);
	//read_usb_vcp();
